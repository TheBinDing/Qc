...
(function ($) {
	...
	$.addFlex = function (t, p) {
		...
		p = $.extend({ //apply default properties
			...
			storageName: '',
			defaultColumnPos: [],
			clearStorageMsg: 'Are you sure you want to reset all your setting?',
			nomsg: 'No items',
		...
		var g = {
			...
			rePosDrag: function () {
				...
				$('thead tr:first th:visible', this.hDiv).each(function () {
					...
					if((browser.msie && browser.version < 11.0) || browser.chrome){
						cdpos += 1;
					}
					if (isNaN(cdpos)) {
						cdpos = 0;
			...
			dragEnd: function () {
				if (this.colresize) {
					...
					if(typeof(Storage) !== 'undefined') {
						var name = '';
						if(p.storageName != ''){
							name = p.storageName+'-width';
							saveWidth = {};
							storageWidth = localStorage.getItem('flexigrid-'+name);
							if(storageWidth != null && storageWidth != 'undefined'){
								saveWidth = JSON.parse(storageWidth);
							}
							saveWidth[p.colModel[n].name] = nw;
							localStorage.setItem('flexigrid-'+name,JSON.stringify(saveWidth));
						}
					}
					...
					if(typeof(Storage) !== 'undefined') {
						...
						$('th', this.hDiv).each(function(){
							var colN = $(this).attr('axis').slice(3);
							columnPos.push(colN);
						});
						var name = '';
						if(p.storageName != ''){
							name = p.storageName+'-columnPos';
							localStorage.setItem('flexigrid-'+name,columnPos);
						}
					}
			...
			toggleCol: function (cid, visible) {
				...
				if(typeof(Storage) !== 'undefined') {
					var name = '';
					if(p.storageName != ''){
						name = p.storageName+'-visible';
						saveVisable = {};
						storageVisable = localStorage.getItem('flexigrid-'+name);
						if(storageVisable != null && storageVisable != 'undefined'){
							saveVisable = JSON.parse(storageVisable);
						}
						if(visible){
							saveVisable['col'+cid] = true;
						} else {
							saveVisable['col'+cid] = false;
						}
						localStorage.setItem('flexigrid-'+name,JSON.stringify(saveVisable));
					}
				}
			...
			clearStorage: function() {
				if(confirm(p.clearStorageMsg)){
					if(typeof(Storage) !== 'undefined') {
						if(p.storageName != ''){
							localStorage.removeItem('flexigrid-'+p.storageName+'-columnPos');
							localStorage.removeItem('flexigrid-'+p.storageName+'-width');
							localStorage.removeItem('flexigrid-'+p.storageName+'-visible');
						}
					}
					...
					$('th', this.hDiv).each(function(){
						...
						var cb = $('input[value=' + colN + ']', g.nDiv)[0];
						$(this).show();
						this.hidden = false;
						$('td[abbr='+name+']').show();
						cb.checked = true;
					...
					for(i in columnPos){
						...
						var div1 = $("th[axis='col" + oldPos[i] + "']", this.hDiv);
						var div2 =$("th[axis='col" + columnPos[i] + "']", this.hDiv);
						if(oldPos[i] != columnPos[i]){
							div1.before(div2);
							this.switchCol2(oldPos[i], columnPos[i]);
						}
					}
					for(i in p.colModel){
						var divWidthTH = $('th[abbr='+p.colModel[i]['name']+']');
						divWidthTH = $('div',divWidthTH);
						divWidthTH.css('width',p.colModel[i]['dwidth']+'px');

						var divWidthTD = $('td[abbr='+p.colModel[i]['name']+']');
						divWidthTD = $('div',divWidthTD);
						divWidthTD.css('width',p.colModel[i]['dwidth']+'px');
						g.addTitleToCell(divWidthTD);

						p.colModel[i]['width'] = p.colModel[i]['dwidth'];
					}
					this.rePosDrag();
				}
			},
			colReorder: function() {
				...
				if(typeof(Storage) !== 'undefined') {
					...
					if(p.storageName != ''){
						name = p.storageName+'-columnPos';
						var columnPos = localStorage.getItem('flexigrid-'+name);
						if(columnPos != null && columnPos != 'undefined'){
							columnPos = columnPos.split(',');
							for(i in columnPos){
								var oldPos = new Array();
								$('th', this.hDiv).each(function(){
									var colN = $(this).attr('axis').slice(3);
			...
			colVis: function() {
				if(typeof(Storage) !== 'undefined') {
					...
					if(p.storageName != ''){
						...
						storageVisable = localStorage.getItem('flexigrid-'+name);
						if(storageVisable != null && storageVisable != 'undefined'){
							storageVisable = JSON.parse(storageVisable);
							for(key in storageVisable){
								var divVis = $('th[axis='+key+']',g.hDiv)[0];
								var cid = key.slice(3);
								var cb = $('input[value=' + cid + ']', g.nDiv)[0];
								var visible = storageVisable[key];
								if (visible) {
									$(divVis).show();
									divVis.hidden = false;
									cb.checked = true;
			...
			switchCol2: function (cdrag, cdrop) {
				//switch order in nDiv
				if (cdrag != cdrop) {
					$("tr[axis='ncol" + cdrag + "']", this.nDiv).before($("tr[axis='ncol" + cdrop + "']", this.nDiv));
				}
				if (browser.msie && browser.version < 7.0) {
					$("tr[axis='ncol" + cdrag + "'] input", this.nDiv)[0].checked = true;
				}
				this.hDiv.scrollLeft = this.bDiv.scrollLeft;
			},
			scroll: function () {
			...
			for (var i = 0; i < p.colModel.length; i++) {
				...
				if( cm ) {	// only use cm if its defined
					...
					if(typeof(Storage) !== 'undefined') {
						...
						if(p.storageName != ''){
							name = p.storageName+'-width';
							var storageWidth = localStorage.getItem('flexigrid-'+name);
							if(storageWidth != null && storageWidth != 'undefined'){
								storageWidth = JSON.parse(storageWidth);
								if(storageWidth[cm.name] != null && storageWidth[cm.name] != 'undefined'){
									cm.width = storageWidth[cm.name];
								}
							}
						}
					}
		...
		if ($('th', g.hDiv).length) {
			...
			$('th div', g.hDiv).each(function () {
				...
				$('tbody', g.nDiv).append('<tr axis="ncol'+ cn + '"><td class="ndcol1"><input type="checkbox" ' + chk + ' class="togCol" value="' + cn + '" /></td><td class="ndcol2">' + this.innerHTML + '</td></tr>');
				cn++;
			});
